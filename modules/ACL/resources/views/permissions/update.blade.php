...
@section('content')
    <ol class="breadcrumb">
        ...
        <li>
            <a href="{{route('permission.index')}}">{{__('table.permissions')}}</a>
        </li>
        <li class="active">
    ...
    <div class="row">
        <form action="{{route('permissions.update', $permission->id)}}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group col-lg-6">
                ...
                <input  class="form-control" name="name" value="{{$permission->name}}" id="name">
            </div>

            <div class="form-group col-lg-6">
                <label for="display_name">{{trans('label.display_name')}}</label>
                <input  class="form-control" value="{{$permission->display_name}}" name="display_name"
                       id="display_name">
            </div>
            ...
            <div class="form-group col-lg-12">
                ...
                <div>
                    <div class="make-switch switch-small" data-on-label="<i class='entypo-check'></i>"
                        ...
                        <input type="checkbox" name="is_active" @if($permission->is_active == 1) checked
                               @endif value="1"/>
                    </div>
                </div>
            </div>
